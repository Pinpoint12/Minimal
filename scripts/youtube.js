...
(function() {
	...
	/* Prevent flash of unstyled content - inject immediately */
	...
	function activateTheaterMode() {
		const theaterBtn = document.getElementsByClassName('ytp-size-button')[0];
		if (theaterBtn) {
			const path = theaterBtn.getElementsByTagName('path')[0];
			if (path?.getAttribute('d') === 'm 28,11 0,14 -20,0 0,-14 z m -18,2 16,0 0,10 -16,0 0,-10 z') {
				theaterBtn.click();
			}
		}
	}
	...
	function deactivateAutoplay() {
		const autoplayBtn = document.getElementsByClassName('ytp-autonav-toggle-button')[0];
		...
		if (!deactivateAutoplay()) {
			...
			if (container) {
				const observer = new MutationObserver(() => {
					if (deactivateAutoplay()) observer.disconnect();
				});
				observer.observe(container, { attributes: true, childList: true, subtree: true, attributeFilter: ['aria-checked'] });
			}
		}
	}
	...
	function replaceSubscriptionManager() {
		if (!document.getElementById('avatar-btn') &&
			!document.getElementById('yt-masthead-account-picker') &&
			!document.querySelector('yt-img-shadow.ytd-topbar-menu-button-renderer')) {
			return;
		}

		const findSectionByText = (selector, text) => {
		...
		let subGuide = findSectionByText('ytd-guide-section-renderer h3, ytd-guide-section-renderer #endpoint-title', 'Subscriptions');
		...
		const subscriptionContainer = subGuide.querySelector('ytd-guide-collapsible-section-entry-renderer') ||
		...
		const subGuideTitle = subGuide.querySelector('h3') ||
			subGuide.querySelector('#endpoint-title') ||
			subGuide.getElementsByTagName('h3')[0];
		if (!subGuideTitle) return;

		const formattedStr = subGuideTitle.querySelector('yt-formatted-string') || subGuideTitle.querySelector('span');
		if (formattedStr) {
	...
	function hideLiveChat() {
		...
		if (!document.getElementById('chatframe')) return;
		...
		const checkInterval = setInterval(() => {
			...
			if (chatFrame?.contentDocument?.readyState === 'complete') {
				...
				if (showHideBtn) {
					const chatButton = showHideBtn.getElementsByTagName('button')[0];
	...
	function addRecommendedVideoToggle() {
		const homePage = document.querySelector('[page-subtype="home"]');
		if (!homePage) return false;

		const children = document.querySelectorAll('[page-subtype="home"]>*');
		...
		btn.textContent = chrome.i18n.getMessage('toggleOn') + ' ' + chrome.i18n.getMessage('videoRecommendations');
		...
		btn.addEventListener('click', (e) => {
			...
			e.target.textContent = (isHidden ? chrome.i18n.getMessage('toggleOff') : chrome.i18n.getMessage('toggleOn')) +
				' ' + chrome.i18n.getMessage('videoRecommendations');
		});
	...
	function addRecommendedVideoToggleTry() {
		if (videoToggleRetries++ > 20) return;
		if (!addRecommendedVideoToggle()) {
			setTimeout(addRecommendedVideoToggleTry, 500);
		}
	}

	/* Replace YouTube homepage with minimal search - U2 C2 */
	function replaceHomePage() {
		...
		<div class="home-container">
			<div class="logo">
				<svg viewBox="0 0 90 20" preserveAspectRatio="xMidYMid meet" focusable="false" style="pointer-events:none;display:block;width:100%;height:100%;">
		...
		style.id = 'minimal-youtube-homepage';
		style.textContent = `
			body { margin:0; padding:0; font-family:Arial,sans-serif; background:${isDarkTheme ? '#0f0f0f' : '#fff'}; color:${isDarkTheme ? '#fff' : '#000'}; display:flex; justify-content:center; align-items:center; height:100vh; }
			.home-container { display:flex; flex-direction:column; align-items:center; }
			.logo { width:120px; margin-bottom:30px; }
			.logo-text path { fill:${isDarkTheme ? '#fff' : '#000'}; }
			.search-group { display:flex; align-items:center; }
			.search-input { width:400px; padding:12px 15px; font-size:16px; border:1px solid ${isDarkTheme ? '#303030' : '#ccc'}; border-radius:24px 0 0 24px; outline:none; background:${isDarkTheme ? '#121212' : '#fff'}; color:${isDarkTheme ? '#fff' : '#000'}; }
			.search-btn { padding:12px 20px; border:1px solid ${isDarkTheme ? '#303030' : '#ccc'}; border-left:none; border-radius:0 24px 24px 0; background:${isDarkTheme ? '#222' : '#f8f8f8'}; cursor:pointer; }
			.search-icon-container { width:20px; height:20px; }
			.search-icon-container svg { fill:${isDarkTheme ? '#fff' : '#000'}; }
		`;
	...
	function init() {
		chrome.storage.sync.get({ [SITE_NAME]: 'enabled' }, (data) => {
			...
			/* Minimal is enabled - apply FOUC prevention and run modifications */
