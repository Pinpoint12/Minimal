...
/* - Hide <body> at document_start so the real feed never flashes before the
     overlay is in place - C2 U1 */
...
(function() {
	...
	/* YouTube wordmark + play icon. The icon <g> keeps YouTube red; the wordmark
	   <g> inherits the overlay's ink colour via the shared design system so it
	   reads as quiet type, not branding - P1 */
	const LOGO_HTML = `
		<svg class="minimal-logo-icon" viewBox="0 0 28.57 20" preserveAspectRatio="xMidYMid meet" focusable="false" aria-hidden="true">
	...
	function replaceSubscriptionManager() {
		...
		/* /subscription_manager was retired by YouTube; the live channels feed is the
		   current equivalent - U3 */
		subManagerLink.href = '/feed/channels';
	...
	function showHomePage() {
		if (homepageOverlay) return;
		homepageOverlay = MinimalCore.mountSearchOverlay({
			id: 'minimal-youtube-homepage',
			appShellSelectors: ['#page-manager', '#masthead-container'],
			logoHTML: LOGO_HTML,
			placeholder: 'Search',
			buildSearchUrl: q => '/results?search_query=' + encodeURIComponent(q)
		});
	}

	/* - Remove homepage overlay on SPA navigation away from / - U2 */
	function hideHomePage() {
		if (!homepageOverlay) return;
		homepageOverlay.remove();
		homepageOverlay = null;
	}
	...
	function setupTitleObserver() {
		...
		if (!titleEl) return;
		/* Observe ONLY the <title> node, never <head> — head churns constantly as
		   YouTube injects/removes scripts and styles - C3 */
		titleObserver = new MutationObserver(removeUnreadCountFromTitle);
	...
	function init() {
		chrome.storage.sync.get({ [SITE_NAME]: 'enabled' }, (data) => {
			...
			/* Single SPA-nav handler — YouTube fires yt-navigate-finish on every route
			   change; MinimalCore also covers pushState + back/forward and gates on a
			   real pathname change. No polling, no replaceState patching - U2 */
			...
			/* Disconnect observers when the page is torn down so they never leak. */
